...
def setup_python_env(project_dir):
    ...
    if not os.path.exists(venv_dir):
        print(f"[POST-UPLOAD] Creating virtual environment in {venv_dir}...")
        import venv
        venv.create(venv_dir, with_pip=True)
    
    # Check if libraries are installed (checking for customtkinter as a proxy)
    # or just run install to be safe (pip handles "already satisfied")
    print(f"[POST-UPLOAD] Syncing dependencies from {req_file}...")
    subprocess.run([venv_python, "-m", "pip", "install", "--quiet", "-r", req_file], check=True)
    return venv_python

def launch_monitor_and_app(source, target, env):
    print("==================================================")
    print("[POST-UPLOAD] Launching Monitor & Desktop App...")
    print("==================================================")
    
    project_dir = env.get("PROJECT_DIR")
    app_path = os.path.join(project_dir, "desktop_app", "main.py")
    
    try:
        python_exe = setup_python_env(project_dir)
    except Exception as e:
        print(f"[WARN] Failed to setup .venv: {e}. Falling back to system python.")
        python_exe = "python"

    import subprocess
    print(f"[POST-UPLOAD] Starting App with: {python_exe}")
    subprocess.Popen([python_exe, app_path], creationflags=subprocess.CREATE_NEW_PROCESS_GROUP | subprocess.DETACHED_PROCESS)
    
    # Start Serial Monitor (this is blocking)
    port = env.get("UPLOAD_PORT")
...
env.AddPostAction("upload", launch_monitor_and_app)
