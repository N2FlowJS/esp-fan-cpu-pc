...
static void handleBeacon(uint8_t fc, const uint8_t* payload, int len, int rssi) {
    ...
    if (!isRecentlySeen(bssid, true, rssi, ssid, info.channel, info.security)) {
        const char* ftype = (fc == 0x80) ? "BEACON" : "PROBE RESP";
        Serial.printf("[SNIFFER] [%s] SSID:%-20s BSSID:%02X:%02X:%02X:%02X:%02X:%02X ch:%d %s\n",
                      ftype, ssid.c_str(),
                      bssid[0],bssid[1],bssid[2],bssid[3],bssid[4],bssid[5],
                      info.channel, info.security.c_str());
    }
}
...
static void handleAssocRequest(const uint8_t* payload, int len, int rssi) {
    String ssid = parseSSID(payload, len, 28);
    const uint8_t* cli = &payload[10];
    ...
    String info_str = "Assoc req → AP:" + String(ap_str)
                    + "  SSID:" + (ssid.length() ? ssid : "?");
    addPacketLog("ASSOC REQ", "Req", cli_str, ap_str, rssi, len, info_str, nullptr, nullptr, payload, len);

    if (!isRecentlySeen(cli, false, rssi, ssid)) {
        Serial.printf("[SNIFFER] [ASSOC REQ] %s -> %s ssid:%s\n",
                      cli_str, ap_str, ssid.length() ? ssid.c_str() : "?");
    }
}

// ── Association Response ──────────────────────────────────────────────────────

static void handleAssocResponse(const uint8_t* payload, int len, int rssi) {
    if (len < 30) return;
    ...
    String info_str = "Assoc Response: " + outcome + "  AP:" + ap_str;
    addPacketLog("ASSOC RESP", outcome, ap_str, cli_str, rssi, len, info_str, nullptr, nullptr, payload, len);
}
...
void dispatchMgmtFrame(uint8_t fc, const uint8_t* payload, int len, int rssi) {
    switch (fc) {
        ...
        case 0x00: // Association Request
            handleAssocRequest(payload, len, rssi);
            break;
