...
static void handleBeacon(uint8_t fc, const uint8_t* payload, int len, int rssi) {
    ...
    bool isNew = !isRecentlySeen(bssid, true, rssi, ssid, info.channel, info.security, info.wifiGen, info.clients, info.utilization);
    
    if (isNew) {
        const char* ftype = (fc == 0x80) ? "BEACON" : "PROBE RESP";
        char bssid_str[18]; macToStr(bssid_str, bssid);
        
        String info_str = "SSID: " + ssid + "  ch:" + String(info.channel) + "  " + info.security;
        if (info.clients >= 0) info_str += "  Clients: " + String(info.clients);
        
        addPacketLog(ftype, "Announce", bssid_str, "Broadcast", rssi, len, info_str, nullptr, nullptr, payload, len);

        snifferLog("[SNIFFER] [%s] SSID:%-20s BSSID:%s ch:%d %s %s",
                      ftype, ssid.c_str(), bssid_str,
                      info.channel, info.security.c_str(), info.wifiGen.c_str());
    }
}
...
static void handleProbeRequest(const uint8_t* payload, int len, int rssi) {
    s_probeReqCount = s_probeReqCount + 1;
    ...
    if (!isRecentlySeen(cli, false, rssi, ssid)) {
        snifferLog("[SNIFFER] [PROBE REQ] %s rssi=%d ssid:%s",
...
static void handleAssocRequest(uint8_t fc, const uint8_t* payload, int len, int rssi) {
    ...
    if (!isRecentlySeen(cli, false, rssi, ssid)) {
        snifferLog("[SNIFFER] [%s] %s -> %s ssid:%s",
...
static void handleDeauth(uint8_t fc, const uint8_t* payload, int len, int rssi) {
    s_deauthCount = s_deauthCount + 1;
    s_rollingDeauthCount = s_rollingDeauthCount + 1;
    if (len < 26) return;
    ...
    snifferLog("[SNIFFER] [%s] ch=%d rssi=%d %s->%s reason:%s",
