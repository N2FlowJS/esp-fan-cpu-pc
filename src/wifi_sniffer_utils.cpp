    ...
    uint32_t now = millis();
    {
        ...
        for (auto& dev : s_devices) {
            if (memcmp(dev.mac, mac, 6) == 0 && dev.isAP == isAP) {
                ...
                // If not recently seen, or if significant info changed, update UI
                if (diff >= 15000) {
                    addDeviceUpdate(dev);
                }
                return diff < 15000;
        ...
        // New device (inside same lock to be safe)
        SnifferDevice d;
        memcpy(d.mac, mac, 6);
        d.ssid     = ssid;
        d.rssi     = rssi;
        ...
        // Keep a larger cache for deduplication (ESP32-S3 has enough RAM)
        if (s_devices.size() >= 100) s_devices.erase(s_devices.begin());
        s_devices.push_back(d);

        // Broadcast new device to UI
        addDeviceUpdate(d);
    }
    return false;
}
...
String getMacVendor(const uint8_t* mac) {
    ...
    if (mac[0] & 0x02) return "Randomized MAC";
    ...
    switch (oui) {
        ...
        case 0x000A27: case 0x000C29: case 0x005056: return "Apple";
...
/**
 * updateDeviceVendor
 * Updates a device's vendor/friendly name (e.g. from DHCP hostname or mDNS)
 */
void updateDeviceVendor(const char* macStr, const String& vendor) {
    if (!macStr || vendor.length() == 0) return;
    uint8_t mac[6];
    if (sscanf(macStr, "%02hhX:%02hhX:%02hhX:%02hhX:%02hhX:%02hhX", 
               &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]) != 6) return;
    
    std::lock_guard<std::mutex> lock(s_snifferMutex);
    for (auto& dev : s_devices) {
        if (memcmp(dev.mac, mac, 6) == 0) {
            // Priority: Don't overwrite if current vendor is already specific (long)
            // but allow updating if current is generic/short.
            if (dev.vendor.length() < vendor.length() || 
                dev.vendor == "" || dev.vendor == "Unknown Vendor" || 
                dev.vendor == "Randomized MAC" || dev.vendor == "Espressif") {
                dev.vendor = vendor;
                addDeviceUpdate(dev);
            }
            return;
