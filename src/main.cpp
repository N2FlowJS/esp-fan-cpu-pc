...
static void startWiFi() {
    ...
    Serial.printf("  [AP] Security: WPA2 (Password: %s)\n", WIFI_AP_PASS);
    Serial.printf("  [AP] IP      : http://%s\n", WIFI_AP_IP);

    // --- 2. Connect to STA network (if configured and enabled) ---
    Preferences prefs;
    prefs.begin("wifi", false); // false = read-write (creates namespace if not exists)
    String sta_ssid = "";
    String sta_pass = "";
    bool sta_enabled = prefs.getBool("enabled", true); // Default to enabled
    ...
    if (sta_enabled && sta_ssid.length() > 0) {
...
void setup() {
    ...
    // Wait for Serial to be ready (for ESP32-S3 USB CDC)
    ...
    Serial.printf("  [HW] Chip: %s, Revision: %d\n", ESP.getChipModel(), ESP.getChipRevision());
    Serial.printf("  [HW] Flash: %d MB, Mode: %d\n", ESP.getFlashChipSize() / (1024 * 1024), ESP.getFlashChipMode());
    Serial.println("========================================");
...
void loop() {
    ...
    if (millis() - s_lastHeartbeat >= 5000) {
        s_lastHeartbeat = millis();
        Serial.printf("[SYS] Uptime: %lu s | Heap: %lu | RSSI: %d\n", 
                      millis() / 1000, ESP.getFreeHeap(), WiFi.status() == WL_CONNECTED ? WiFi.RSSI() : 0);
    }
    ...
    if (Serial.available() > 0) {
        ...
        if (cmd == "pcap_start") {
            ...
            webServerInvalidateSession();
        } else if (cmd.startsWith("set_host_temp")) {
            String arg = cmd.substring(13);
            arg.trim();
            if (arg.length() > 0) {
                float t = arg.toFloat();
                ...
                Serial.println("[ERR] Usage: set_host_temp <float>");
            }
